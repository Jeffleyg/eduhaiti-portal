...
export class ForumsService {
  ...
  private async ensureClassAccess(classId: string, requester: Requester) {
    ...
    if (!cls) {
      throw new ForbiddenException(
        'You do not have access to this class forum',
      );
    }
  }
  ...
  async listThreads(classId: string, requester: Requester) {
    ...
    const userIds = [
      ...new Set(parsed.map((item) => item.createdById).filter(Boolean)),
    ] as string[];
    ...
    const postCountMap = new Map(
      postCounts.map((item) => [item.entityId, item._count._all]),
    );

    return parsed.map((item) => ({
      ...item,
      createdBy: item.createdById
        ? (userMap.get(item.createdById) ?? null)
        : null,
  ...
  async listPosts(threadId: string, requester: Requester) {
    ...
    const userIds = [
      ...new Set(posts.map((item) => item.createdById).filter(Boolean)),
    ] as string[];
    const users = userIds.length
      ? await this.prisma.user.findMany({
          where: { id: { in: userIds } },
    ...
    return {
      ...
      posts: posts.map((post) => ({
        ...
        createdBy: post.createdById
          ? (userMap.get(post.createdById) ?? null)
          : null,
      })),
    };
  }
