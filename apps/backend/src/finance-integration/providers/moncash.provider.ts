...
@Injectable()
export class MonCashProvider implements MobileMoneyProvider {
  readonly name = 'moncash' as const;
  ...
  async processCharge(
    ...
    const endpoint = this.config.get<string>('MONCASH_API_URL');
    const apiKey = this.config.get<string>('MONCASH_API_KEY');
    const timeoutMs = Number(
      this.config.get<string>('MONCASH_TIMEOUT_MS') ?? '4500',
    );
    const fallbackFeePercent = Number(
      this.config.get<string>('MONCASH_FEE_PERCENT') ?? '0.009',
    );

    if (!endpoint || !apiKey) {
      throw new ServiceUnavailableException(
        'MonCash provider configuration missing',
      );
    }
    ...
    try {
      ...
      if (!response.ok) {
        throw new BadGatewayException(
          `MonCash rejected charge (${response.status})`,
        );
      }
      ...
      return {
        ...
        providerTransactionId:
          providerTransactionId || `MON-${request.externalReference}`,
        netAmountHtg,
        feeAmountHtg,
      };
    } catch (error) {
      ...
      throw new BadGatewayException('MonCash provider unavailable');
    } finally {
