...
export class GradesService {
  ...
  async create(
    ...
    if (
      ...
      throw new ForbiddenException(
        'Teacher can only manage grades for own classes',
      );
    }
    ...
    if (
      ...
      throw new BadRequestException(
        'Invalid numeric values for score, maxScore or weight',
      );
    }
    ...
    if (
      ...
      throw new BadRequestException(
        'Academic year does not match the selected class',
      );
    }
  ...
  async getStudentReport(studentId: string, academicYearId: string) {
    // Fetch published grades with related discipline and class (including teacher)
    const grades = await this.prisma.grade.findMany({
    ...
    // Build per-discipline class statistics (min, max, avg) and enrich grades
    const enrichedGrades = [] as any[];
    for (const g of grades) {
      // aggregate for the class+discipline (published only)
      ...
      enrichedGrades.push({
        ...
        // placeholder for teacher appreciation/comments (can be extended)
        appreciation: null,
      });
    }
    ...
    // Attendance summary for the academic year (counts)
    ...
    // School info from student's record (if set)
  ...
  async getStudentEvolution(studentId: string, academicYearId?: string) {
    ...
    for (const grade of grades) {
      const normalized =
        grade.maxScore > 0 ? (grade.score / grade.maxScore) * 20 : 0;
    ...
    const timeline = [...monthlyMap.entries()]
      ...
      .map(([period, value]) => ({
        period,
        average:
          value.count > 0 ? Number((value.total / value.count).toFixed(2)) : 0,
        samples: value.count,
      }));
    ...
    const overallAverage =
      timeline.length > 0 ? Number((total / timeline.length).toFixed(2)) : 0;
