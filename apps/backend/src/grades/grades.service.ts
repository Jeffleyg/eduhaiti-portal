...
export class GradesService {
  ...
  async create(payload: {
    ...
    if (!Number.isFinite(payload.score) || !Number.isFinite(maxScore) || !Number.isFinite(weight)) {
      throw new BadRequestException("Invalid numeric values for score, maxScore or weight")
    }
    ...
    if (payload.academicYearId && payload.academicYearId !== classData.academicYearId) {
      throw new BadRequestException("Academic year does not match the selected class")
    }
